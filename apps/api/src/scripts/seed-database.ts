...
import { db, initializeDatabase, wipeDatabase, queries } from '../db/index.js';

const __dirname = dirname(fileURLToPath(import.meta.url));
// Product images live in the web app's public folder (served at /images/... by Vite/Express static).
const IMAGES_ROOT = join(__dirname, '../../../web/public/images');
const SEED_JSON_PATH = join(__dirname, '../../../../productseed.json');
...
function getRandomImages(
	...
	try {
		const imagesPath = join(IMAGES_ROOT, categorySlug);
...
function loadSeedData(): SeedData {
	if (existsSyncSafe(SEED_JSON_PATH)) {
		try {
			const rawData = readFileSync(SEED_JSON_PATH, 'utf-8');
	...
	for (const category of APP_CONFIG.categories) {
		const productCount = Math.floor(Math.random() * 11) + 10;
...
export function seedDatabase() {
	...
	try {
		...
		const usedFAQProductPairs = new Set<string>();
		...
		for (let i = 0; i < seedData.faqs.length; i++) {
			...
			let linkedCount = 0;
			const maxLinks = Math.min(categoryProducts.length, Math.floor(Math.random() * 3) + 1);
